...
double processPackets(Sketch* skt, vector<pair<pair<uint32_t, uint32_t>, uint8_t>>& dataset) {
    ...
    //skt->clean(); // move to saveresult(), because after all period, the sketch must conduct clean()
...
void saveResults(string outputFilePath, Sketch* skt, unordered_map<uint8_t, unordered_map<uint32_t, std::unordered_set<uint32_t>>>& realFlowInfo, uint32_t T, uint32_t K, string name, double throughput) {
    ...
    clock_t current = clock();
    double detection_time = ((double)current - start) / CLOCKS_PER_SEC;//std::chrono::duration_cast<std::chrono::microseconds>(current - start);
//     if (strcmp(name.c_str(), "PSSBM") == 0 || strcmp(name.c_str(), "PSSHLL") == 0 || strcmp(name.c_str(), "KTSketch") ) {
...
void run(uint32_t mem, uint32_t detectT, uint32_t detectK, uint8_t total_periods, string name, uint32_t d, float ratio) {
    //prepare the dataset
    cout << "prepare the dataset" << endl;
    string dataDir = R"(../../../../ZSL/dataset/CAIDA2016_600wPerTXT/)";
    ...
    for (int i = 0; i < total_periods; ++i) {
        ...
        filename_stream << dataDir << std::setfill('0') << i << "_ipv4" << ".txt";
    ...
    if (strcmp(name.c_str(), "PSSBM") == 0) {
        ...
        string outputFilePath = "../Params/PSS_BM_" + std::to_string(mem) + "_" + std::to_string(detectT) + "_" + std::to_string(detectK) + "_" + std::to_string(d) + "_" + std::to_string(ratio) + ".txt";
    ...
    if (strcmp(name.c_str(), "PSSHLL") == 0) {
        ...
        cout << endl;
        cout << "save the result in txt ..." << endl;
        string outputFilePath = "../records_CAIDA16/PSS_HLL_" + std::to_string(mem) + "_" + std::to_string(detectT) + "_" + std::to_string(detectK) + ".txt";
        saveResults(outputFilePath, skt, realFlowInfo, detectT, detectK, name, throughput);
    }
    ...
    if (strcmp(name.c_str(), "KTSketch") == 0) {
        ...
        cout << endl;
        cout << "save the result in txt ..." << endl;
        string outputFilePath = "../records_CAIDA16/KTSketch_" + std::to_string(mem) + "_" + std::to_string(detectT) + "_" + std::to_string(detectK) + ".txt";
    ...
    if (strcmp(name.c_str(), "HLLsampler") == 0) {
        ...
        cout << endl;
        cout << "save the result in txt ..." << endl;
        string outputFilePath = "../records_CAIDA16/HLLsampler_" + std::to_string(mem) + "_" + std::to_string(detectT) + "_" + std::to_string(detectK) + ".txt";
        saveResults(outputFilePath, skt, realFlowInfo, detectT, detectK, name, throughput);
    }
    ...
    if (strcmp(name.c_str(), "SpreadSketch") == 0) {
        ...
        cout << endl;
        cout << "save the result in txt ..." << endl;
        string outputFilePath = "../records_CAIDA16/SpreadSketch_" + std::to_string(mem) + "_" + std::to_string(detectT) + "_" + std::to_string(detectK) + ".txt";
        saveResults(outputFilePath, skt, realFlowInfo, detectT, detectK, name, throughput);
    }
}
...
int main(int argc, char *argv[]) {
    //run(256, 50, 10, 12);
    std::vector<uint32_t> d_vt = {1, 2, 3, 4, 5};
    std::vector<float> ratio_vt = {0.1, 0.3, 0.5, 0.7, 0.9};
    for (auto iter_d = d_vt.begin(); iter_d != d_vt.end(); iter_d ++) {
        for (auto iter_ratio = ratio_vt.begin(); iter_ratio != ratio_vt.end(); iter_ratio ++) {
            if (strcmp(argv[1], "PSSBM") == 0) {
               run(std::stoi(argv[2]), std::stoi(argv[3]), std::stoi(argv[4]), std::stoi(argv[5]), "PSSBM", *iter_d, *iter_ratio); 
            }
        }
    }
    return 0;
