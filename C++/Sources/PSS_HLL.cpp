...
PSSHLL::PSSHLL(uint32_t m, uint32_t d, uint32_t w, uint32_t t) {
    this->m = m;
    this->d = d;
    this->w = w;
    this->threshold_t = t;
...
void PSSHLL::insert(uint32_t key, uint32_t ele, uint8_t _period) {
    uint32_t hash_index, hash_value, leftmost = 0;
    ...
    MurmurHash3_x86_32(hash_input_xor, 8, sample_seed, &hash_value);
    while (hash_value) {
        hash_value >>= 1;
        leftmost ++;
    }
    ...
    if (leftmost > R[hash_index]) {
        inc = 1.0 / q;
        q = q - pow(0.5, R[hash_index]) / (1.0 * m) + pow(0.5, leftmost) / (1.0 * m);
...
void PSSHLL::post_process() {
    return;
}

std::unordered_set<uint32_t> PSSHLL::detect(uint32_t thr) {
...
uint32_t PSSHLL::query(uint32_t key) {
