...
const int BIT_LEN = 2097152;
const int CELL_NUM = 8738;
const int BLOCK_SIZE = 32;

__device__ uint8_t kernel_B[BIT_LEN];
__device__ uint32_t kernel_cnt_zero;
...
__device__ inline void kernel_update(uint32_t key, uint32_t ele, uint8_t _period) {
    ...
    uint32_t B_index = (hash1(key) ^ hash1(ele)) % BIT_LEN;
    if (kernel_B[B_index] == 0) {
        //atomicOr(kernel_B + B_index, 1);
        kernel_B[B_index] = 1;
        float inc = (1.0 * BIT_LEN) / (kernel_cnt_zero * 1.0);
        ...
        while (next && lock_requests --) {
            ...
            if (status == 0) {
                ...
                if (empty_index != -1) {
                    ...
                    /*if (kernel_Weights[min_index] == 0) {
                        Pr = (1.0 * inc) / (1.0 * inc + kernel_Cards[min_index]);
                    } else {
                        Pr = MIN((1.0 * _period - 1.0 * kernel_Times[min_index]) / (1.0 * kernel_Weights[min_index]), 1.0) * (1.0 * inc) / (1.0 * inc + kernel_Cards[min_index]);
                    }*/
